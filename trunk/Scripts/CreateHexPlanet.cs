...
public class CreateHexPlanet : MonoBehaviour {
	...
	void Start () {
		...
		if (this.GetComponentInChildren<MainMenu>().loadingWord)
		{
			planetFile =	new System.IO.StreamReader("Perontor.txt");
			string myString = planetFile.ReadLine();
			numSubDivides = int.Parse(myString);
		}
		else
		{
			numSubDivides = this.GetComponentInChildren<MainMenu>().worldSize;
		}
		...
		planet.NewPlanet(numSubDivides, numFactions);
	...
	public float[] LoadPlanet()
	{
		...
		for (int v=0; v<nmbrHexs; v++)
		{
			...
			//myString = myFile.ReadLine();
			//terrains[v] = Terrains.myString;
		}	
		return alts;
	}
		
	private float[] CreateTerrain(float scale)
	{
	...
	private bool PlaceTiles(float[] alts)
	{
		GameObject tile;
		if (this.GetComponentInChildren<MainMenu>().loadingWord)
		{
	...
	private List<int> GetNbringTiles(int h)
	{	
	...
	private void ApplyTerrainTexture(Tile tile, int h, Vector3 m, float altitude)
	{
		...
		{
			tile.GetComponent<MeshRenderer>().material = (Material)Resources.Load("TerrainMaterials/Tile_Water", typeof(Material));
			tile.GetComponent<Tile>().terrain = Terrains.WATER;					
		}
		else if (Mathf.Abs(m.y) > articCircle)
		{
			tile.GetComponent<MeshRenderer>().material = (Material)Resources.Load("TerrainMaterials/Tile_Ice", typeof(Material));
			tile.GetComponent<Tile>().terrain = Terrains.ICE;
		}
		else if(Mathf.Abs(m.y) < 0.4f)
		{
			tile.GetComponent<MeshRenderer>().material = (Material)Resources.Load("TerrainMaterials/Tile_Sand", typeof(Material));
		...
		else
		{
			if (Random.value<0.5f)
			{
				...
				tile.GetComponent<Tile>().GrowTree();
			}
		}
	}
...
//		Debug.Log(edges.Count.ToString() + "edges found");
	...
	private int[] CreateTileTriangles(Vector3[] v)
	{
