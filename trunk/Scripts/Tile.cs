...
//		Debug.Log("trying to move chit");
		...
		chit.faction.characters[chit.faction.turn].remainingCR--;
	}
...
//				Debug.Log("too far away :(");		
	...
	public void ClickedOnATileWithAChit()
	{
		...
		{
			Faction currFaction = planet.f[planet.turn.currentFctr];
			if (currFaction.characters[currFaction.turn].remainingCR>0)
			{
//				List<int> selTri = SelectTilesForMoving();			
//				planet.HighlightTiles(selTri.ToArray(), Terrains.move);
				planet.orderingChit = chitOnTile;
	//			Debug.Log("Selecting Chit for movement with range:" + chitOnTile.movementRange.ToString());
	...
	//			Debug.Log("tiles within range: " + tilesWithinRange.Count.ToString());
	...
	public void AttachChit(Faction f, ChitTypes cType, string cname)
	{
		...
		switch (cType)
		{
			case ChitTypes.COMMANDER:
				...
				gameObject.GetComponent<Character>().characterName = cname;
			...
			case ChitTypes.NECTOWER:		
				...
				gameObject.GetComponent<Character>().characterName = "The Necromancer";
				gameObject.GetComponent<Character>().cType = cType;
				break;
			default:
	...
	public void GrowTree()
	{
		...
		//account for rotation
		Vector3 m =  planet.transform.localToWorldMatrix * midpoint;
		GameObject 	gameObject = (GameObject)GameObject.Instantiate(Resources.Load("Tree"), m, Quaternion.LookRotation(midpoint));	
